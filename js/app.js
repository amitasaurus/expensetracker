(function() {
	angular.module('expenseTracker', ['firebase', 'ngRoute']).factory('rootRef', function() {
		...
		$routeProvider.when('/home', {
			template: '<home categories="$resolve.categories" expenses="$resolve.expenses"></home>',
			resolve: {
				expenses: function(fbRef, expenses, auth) {
					return auth.$requireSignIn().then(function() {
						var query = fbRef.getExpenseRef().orderByChild('date');
						return expenses(query).$loaded();
					});
				},
